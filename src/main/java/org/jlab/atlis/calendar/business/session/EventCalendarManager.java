...
public class EventCalendarManager {
  ...
  public EventCalendar getEventCalendar(int calendarId, Date yearMonth) {
    ...
    LOGGER.log(
        Level.FINEST, "EventCalendarManager.getEventCalendar: {0}", format.format(yearMonth));
  ...
  private List<EventCalendarDay> getPaddedDays(List<EventCalendarDay> days) {
    ...
    LOGGER.log(
        Level.FINEST,
        "EventCalendarManager.getPaddedDays: first day of week in month={0}, last day of week in month={1}",
        new Object[] {firstDayOfWeekInMonth, lastDayOfWeekInMonth});
    ...
    LOGGER.log(
        Level.FINEST,
        "EventCalendarManager.getPaddedDays: begin padding days={0}, end padding days={1}, total days={2}",
        new Object[] {beginPadding.size(), endPadding.size(), all.size()});
  ...
  private List<EventCalendarWeek> putDaysIntoWeeks(List<EventCalendarDay> days) {
    ...
    for (int i = 1; i <= numberOfWeeks; i++) {
      ...
      Date lastDayOfWeek =
          week.getDays()
              .get(6)
              .getYearMonthDay(); // Use last day of week to capture week 1 which may overlap into
      // previous year!
  ...
  private List<EventCalendarDay> getDays(int calendarId, Date start, Date end) {
    ...
    LOGGER.log(
        Level.FINEST,
        "EventCalendarManager.getDays: {0} - {1}",
        new Object[] {format.format(start), format.format(end)});
  ...
  private void populateDays(SortedMap<Date, EventCalendarDay> days, List<Occurrence> occurrences) {
    ...
    for (Occurrence occurrence : occurrences) {
      ...
      if (day != null) {
        ...
        LOGGER.log(
            Level.WARNING,
            "Database occurrence: {0} not found in current calendar!",
            occurrence.getYearMonthDay());
      }
    }
  }
}
