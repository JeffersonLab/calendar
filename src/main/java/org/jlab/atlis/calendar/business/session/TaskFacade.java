...
public class TaskFacade extends AbstractFacade<Task> {
  ...
  private Predicate[] getPredicates(
      AtlisSearchFilter filter, CriteriaBuilder builder, CriteriaQuery cq, Root<Task> task) {
    List<Predicate> predicates = new ArrayList<>();

    if (filter.getStart() != null && filter.getEnd() != null) {
      Predicate condition =
          builder.between(task.get(Task_.scheduledDate), filter.getStart(), filter.getEnd());
    ...
    if (filter.getTitlePhrase() != null && !filter.getTitlePhrase().trim().isEmpty()) {
      Predicate condition =
          builder.like(
              builder.upper(task.get(Task_.title)),
              "%" + filter.getTitlePhrase().toUpperCase() + "%");
      predicates.add(condition);
    }
    ...
    if (filter.getLiaisonPhrase() != null && !filter.getLiaisonPhrase().trim().isEmpty()) {
      Predicate condition =
          builder.like(
              builder.upper(task.get(Task_.liaison)),
              "%" + filter.getLiaisonPhrase().toUpperCase() + "%");
      predicates.add(condition);
    }
