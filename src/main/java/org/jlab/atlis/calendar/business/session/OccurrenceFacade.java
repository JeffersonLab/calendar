...
public class OccurrenceFacade extends AbstractFacade<Occurrence> {
  ...
  @RolesAllowed("calendar-admin")
  public void move(
      int fromCalendarId, Date fromStart, int numberOfDays, int toCalendarId, Date toStart) {
    ...
    while (cutIterator.hasNext()) {
      ...
      org.jlab.atlis.calendar.persistence.entity.Calendar cal =
          em.find(org.jlab.atlis.calendar.persistence.entity.Calendar.class, toCalendarId);
  ...
  public List<Occurrence> findExceptHidden(int calendarId, Date start, Date end) {
    TypedQuery<Occurrence> q =
        em.createNamedQuery("Occurrence.findByDateRangeExceptHidden", Occurrence.class);
  ...
  public List<Occurrence> findDay(int calendarId, Date yearMonthDay) {
    TypedQuery<Occurrence> q =
        em.createNamedQuery("Occurrence.findByYearMonthDay", Occurrence.class);
  ...
  public List<Occurrence> findShown(int calendarId, Date yearMonthDay, Shift shift) {
    TypedQuery<Occurrence> q =
        em.createNamedQuery("Occurrence.findShownByYearMonthDayAndShift", Occurrence.class);
  ...
  public List<Occurrence> findHiddenAndMore(int calendarId, Date yearMonthDay, Shift shift) {
    TypedQuery<Occurrence> q =
        em.createNamedQuery("Occurrence.findHiddenAndMoreByYearMonthDayAndShift", Occurrence.class);
  ...
  public List<Occurrence> findInOrder(int calendarId, Date yearMonthDay, Shift shift) {
    TypedQuery<Occurrence> q =
        em.createNamedQuery("Occurrence.findByYearMonthDayAndShiftInOrder", Occurrence.class);
  ...
  private List<OccurrenceStyle> wrapInOccurrenceStyle(
      Occurrence occurrence, List<OccurrenceStyleChoice> styles) {
  ...
  public void batchUpdate(
      ...
      List<BigInteger> selectedOccurrences,
      SelectedOccurrenceFields selectedFields) {
    if (selectedOccurrences.size() > 0 && selectedFields.count() > 0) {
      for (BigInteger id : selectedOccurrences) {
        ...
        if (selectedFields.isStyle()) {
          // Hibernate doesn't like it when you set collection if entity is being managed so we
          // clear and add.
          o.getStyles().clear();
          em.flush(); // If we don't do this then the inserts might occur before deletes and
          // unique constraint would be violated if we're putting the same style back
          // in!
