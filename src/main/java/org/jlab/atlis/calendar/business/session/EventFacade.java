...
public class EventFacade extends AbstractFacade<Event> {
    ...
    public Event findWithOccurrences(Object id) {
        ...
        //We rely on orphan removal feature so invalidating cache isn't necessary anymore! (we now keep both sides of relationship is sync)
        //em.getEntityManagerFactory().getCache().evictAll(); // If we've deleted any occurrences then the cache is wrong!
        if (event != null) {
    ...
    public void createEventWithOccurrences(Occurrence occurrence, Set<DayAndShift> instances) {
        ...
        for (DayAndShift das : instances) {
            ...
            Integer orderId = occurrenceFacade.findNextOrderId(o.getEvent().getCalendar().getCalendarId(), o.getYearMonthDay(), o.getShift());
