...
public class CalendarHelper {
  ...
  public String calculateNextWeekURL(int calendarId, int year, int week) {
    ...
    String url =
        "view-week?calendar=" + calendarId + "&year=" + yearWeek[0] + "&week=" + yearWeek[1];
  ...
  public String calculateNextOutlookURL(int calendarId, int year, int week) {
    ...
    String url =
        "view-outlook?calendar=" + calendarId + "&year=" + yearWeek[0] + "&week=" + yearWeek[1];
  ...
  public String calculateAddEventURL(int calendarId, Date dayMonthYear) {
    ...
    String url =
        "edit-event?calendar=" + calendarId + "&yearMonthDay=" + format.format(dayMonthYear);
  ...
  public String getAbsoluteViewWeekURL(
      HttpServletRequest request, int calendarId, int year, int week) {
    return getAbsoluteBaseURL(request) + "/" + calculateViewWeekURL(calendarId, year, week);
  }

  public String getAbsoluteViewDayURL(
      HttpServletRequest request, int calendarId, Date yearMonthDay) {
  ...
  public String getAbsoluteHostURL(HttpServletRequest request) {
    ...
    /**
     * Generally relative URLS should be used to avoid trying to figure out links, however in some
     * cases you just need an absolute URL.
     *
     * <p>Use cases include: - generate PDF from HTML. Relative link inside PDF are useless -
     * generate Email from HTML. Relative links inside Emails are useless - generate logbook entry.
     * Ditto. - generate redirect to remote login server providing return URL.
     *
     * <p>Note: there is no Servlet API to query for "what is my secure port". In production, it
     * should be 443. Often differs in development. In web.xml you can mark a path as confidential,
     * forcing redirect to secure port, but that doesn't help inside Servlet code. App server knows
     * secure port. Won't tell you. The easiest way is to set an env. Turns out you often need to
     * consider your proxy server as well in the use cases above. Even if app server DID tell you
     * your app server secure port, it might not match outward facing proxy server.
     */
