...
public class EditOccurrence extends HttpServlet {
  ...
  protected void doGet(HttpServletRequest request, HttpServletResponse response)
    ...
    if (occurrenceIdStr != null && !occurrenceIdStr.trim().isEmpty()) { // Edit existing
      ...
      try {
        ...
        throw new CalendarException(
            "Unable to edit occurrence: Invalid occurrenceId: " + e.getMessage());
      }
      ...
      if (occurrence == null) {
        throw new CalendarException(
            "Unable to edit occurrence: Unable to locate occurrence with ID = " + occurrenceId);
      }
    } else if (copyIdStr != null
        && !copyIdStr.trim().isEmpty()) { // Create new by copying an occurrence
      ...
      if (copy == null) {
        throw new CalendarException(
            "Unable to edit occurrence: Unable to locate occurrence to copy with ID = " + copyId);
      }
    ...
    getServletConfig()
        .getServletContext()
        .getRequestDispatcher("/WEB-INF/views/edit-occurrence.jsp")
    ...
    if (create) { // Create New
      ...
      Integer orderId =
          occurrenceFacade.findNextOrderId(
              occurrence.getEvent().getCalendar().getCalendarId(),
