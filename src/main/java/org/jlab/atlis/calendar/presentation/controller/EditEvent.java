 ...
 * @author ryans
 */
@WebServlet(name = "EditEvent", urlPatterns = {"/edit-event"})
public class EditEvent extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (eventIdStr != null && !eventIdStr.trim().isEmpty()) { // Edit existing
            ...
            if (event == null) {
                throw new CalendarException("Unable to edit event: Unable to locate event with ID = " + eventId);
            }
            ...
            if (occurrenceIdStr != null && !occurrenceIdStr.trim().isEmpty()) {
                ...
                try {
                    ...
                    throw new CalendarException("Unable to edit event: Invalid occurrenceId: " + e.getMessage());
                }

                occurrence = occurrenceFacade.find(occurrenceId);

                if (occurrence == null) {
                    throw new CalendarException("Unable to edit event: Unable to locate occurrence with ID = " + occurrenceId);
                }
            }
            ...
            try {
                ...
                throw new CalendarException("Unable to edit event: Invalid yearMonthDay: " + e.getMessage());
            }
        ...
        getServletConfig().getServletContext().getRequestDispatcher("/WEB-INF/views/edit-event.jsp").forward(request, response);
    }
        ...
        if (messages.isEmpty()) {
            if (create) {
                ...
                if (event.getOccurrenceList().size() == selectedOccurrences.size()) {
                    messages.put("selectedOccurrences", "Cannot delete all occurrences - delete the event to do that");
                }
            } else if(batchHide) {
                selectedOccurrences = convertAndValidateSelectedOccurrences(request);
            } else if(batchShow) {
                ...
                if (selectedOccurrences != null) {
                    if (ids == null) {
                        ...
                        for (BigInteger id : selectedOccurrences) {
                            if (!ids.contains(id)) {
                                messages.put("selectedOccurrences", "Event does not have selected occurrence: " + id);
                            }
                        }
                    }
                }
    ...
    private Occurrence convertAndValidateOccurrenceWithSelectedFields(HttpServletRequest request, SelectedOccurrenceFields selectedFields) {
        @SuppressWarnings("unchecked")
        Map<String, String> messages = (Map<String, String>) request.getAttribute("messages");
